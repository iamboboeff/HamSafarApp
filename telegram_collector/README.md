...
`gemini-3.7-flash` с низким уровнем размышления для быстрой обработки; модель
можно сменить через `GEMINI_MODEL`. Телефонные номера удаляются из текста до
отправки в Gemini и добавляются в результат локально.
