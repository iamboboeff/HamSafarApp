...
_CITY_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "Бохтар": ("бохтар", "курган-тюбе", "қӯрғонтеппа", "кургантюбе"),
    "Куляб": ("куляб", "кулоб", "кӯлоб", "kulob"),
    "Панджакент": ("панджакент", "пенджикент", "панчакент", "panjakent"),
...
_OFFER_RE = re.compile(
    ...
    r"меравам|мерем|рафта\s+истодаам|"
    r"кетаман|кетяпман|бораман|йўлга\s+чиқаман)\b",
    re.IGNORECASE,
)
...
_CARGO_RE = re.compile(
    r"\b(?:посылк\w*|посыл\w*|пасил\w*|поссы?л\w*|груз\w*|бор)\b",
    re.IGNORECASE,
)
_SEATS_RE = re.compile(
    r"(?<!\d)(\d{1,2})\s*(?:свободн\w*\s*)?"
    r"(?:мест\w*|нафар|одам\w*|пассажир\w*)\b",
    re.IGNORECASE,
)
